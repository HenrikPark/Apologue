...
namespace AGE
{
	...
	void EditorLayer::OnImGuiRender()
	{
			...
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
			{
				...
				else if (parentPath == "textures")	// Load texture
				{
					...
					//m_Hcomponent.Texture = texture;
						...
						AGE_WARN("Could not load texture {0}", texturePath.filename().string());
				}
			}
		...
		if (selectedEntity && m_GizmoType != -1)
		{
			...
			// Snapping
			bool snap = Input::IsKeyPressed(AGE_KEY_LEFT_CONTROL);
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>(AGE_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	bool EditorLayer::OnKeyPressed(KeyPressedEvent& e)
	{
		...
		bool shift = Input::IsKeyPressed(AGE_KEY_LEFT_SHIFT) || Input::IsKeyPressed(AGE_KEY_RIGHT_SHIFT);
	...
	{
		if (e.GetMouseButton() == AGE_MOUSE_BUTTON_LEFT)
		{
			if (m_ViewportHovered && !ImGuizmo::IsOver() && !Input::IsKeyPressed(AGE_KEY_LEFT_ALT))
	...
	{
		std::string filepath = FileDialogs::OpenFile("AGE Scene (*.AGE)\0*.AGE\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	{
		std::string filepath = FileDialogs::SaveFile("AGE Scene (*.AGE)\0*.AGE\0");
		if (!filepath.empty())
		{
	...
	{
		AGE_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate, "");
